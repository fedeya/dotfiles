...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME background.color=0xfff6c177 label.drawing=no icon.color=$overlay_color icon.padding_right=8
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes background.color=$overlay_color icon.padding_right=0 icon.color=$icon_color label.color=$text_color

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME background.color=0xfff6c177 label.color=$overlay_color icon.color=$overlay_color
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME background.color=0xffeed49f label.drawing=no icon.color=$overlay_color icon.padding_right=8
    else
        sketchybar --set $NAME background.color=0xff31748f
    fi
}
